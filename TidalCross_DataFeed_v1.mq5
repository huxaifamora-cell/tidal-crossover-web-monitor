...
void PushAllToDashboard()
{
   ...
   for(int i = 0; i < total; i++)
   {
      ...
      double spreadPts   = (pt > 0) ? (ask - bid) / pt : 0.0;
      double spreadDollar = (tickSize > 0) ? spreadPts * (tickVal / tickSize) * minLot : 0.0;
      double minMargin   = 0.0;
      OrderCalcMargin(ORDER_TYPE_BUY, sym, minLot, ask, minMargin);
      ...
      batchJSON += StringFormat(
         "{\"id\":\"%s\"," 
          ...
          "{\"timeframes\":%s}",
         id, bid, ask, minLot, minMargin, spreadDollar, overallSignal,
