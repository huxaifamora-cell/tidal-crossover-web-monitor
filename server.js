...
// ─────────────────────────────────────────────
const INDICES = [
  { id:"v5",    name:"Volatility 5 Index",         short:"V5",       vol:"low"    },
  { id:"v10",   name:"Volatility 10 Index",        short:"V10",      vol:"low"    },
  { id:"v15",   name:"Volatility 15 Index",        short:"V15",      vol:"low"    },
  { id:"v25",   name:"Volatility 25 Index",        short:"V25",      vol:"medium" },
  { id:"v30",   name:"Volatility 30 Index",        short:"V30",      vol:"medium" },
  { id:"v50",   name:"Volatility 50 Index",        short:"V50",      vol:"medium" },
  { id:"v75",   name:"Volatility 75 Index",        short:"V75",      vol:"high"   },
  { id:"v90",   name:"Volatility 90 Index",        short:"V90",      vol:"high"   },
  { id:"v100",  name:"Volatility 100 Index",       short:"V100",     vol:"high"   },
  { id:"v5s",   name:"Volatility 5 (1s) Index",    short:"V5(1s)",   vol:"low"    },
  { id:"v10s",  name:"Volatility 10 (1s) Index",   short:"V10(1s)",  vol:"low"    },
  { id:"v15s",  name:"Volatility 15 (1s) Index",   short:"V15(1s)",  vol:"low"    },
  { id:"v25s",  name:"Volatility 25 (1s) Index",   short:"V25(1s)",  vol:"medium" },
  { id:"v30s",  name:"Volatility 30 (1s) Index",   short:"V30(1s)",  vol:"medium" },
  { id:"v50s",  name:"Volatility 50 (1s) Index",   short:"V50(1s)",  vol:"medium" },
  { id:"v75s",  name:"Volatility 75 (1s) Index",   short:"V75(1s)",  vol:"high"   },
  { id:"v90s",  name:"Volatility 90 (1s) Index",   short:"V90(1s)",  vol:"high"   },
  { id:"v100s", name:"Volatility 100 (1s) Index",  short:"V100(1s)", vol:"high"   },
  { id:"v150s", name:"Volatility 150 (1s) Index",  short:"V150(1s)", vol:"high"   },
  { id:"v250s", name:"Volatility 250 (1s) Index",  short:"V250(1s)", vol:"high"   },
];

function defaultLiveState(idx) {
  return {
    bid:null, ask:null,
    minLot:null, minMargin:null, spreadDollar:null,
    ...
    timeframes:[
      ...
      {tf:"M30",status:"watch",label:"Waiting for data",signal:"neutral"},
...
function buildCard(idx,i){
  ...
  const lotStr=live.minLot!=null?Number(live.minLot).toFixed(3):'&#8212;';
  const marginStr=live.minMargin!=null?'$'+Number(live.minMargin).toFixed(2):'&#8212;';
  const spreadStr=live.spreadDollar!=null?'$'+Number(live.spreadDollar).toFixed(4):'&#8212;';
  ...
  return '<div class="card" id="card-'+idx.id+'" data-vol="'+idx.vol+'" data-id="'+idx.id+'" data-active="'+countActiveTFs(live)+'" style="animation-delay:'+i*.04+'s" onclick="toggleCard(this)">'+
    ...
    '<div class="stat-cell"><div class="stat-cell-label">Min Lot</div><div class="stat-cell-val" id="lot-'+idx.id+'">'+lotStr+'</div></div>'+
    '<div class="stat-cell"><div class="stat-cell-label">Min Margin</div><div class="stat-cell-val" id="margin-'+idx.id+'">'+marginStr+'</div></div>'+
    '<div class="stat-cell"><div class="stat-cell-label">Spread $</div><div class="stat-cell-val" id="spread-'+idx.id+'">'+spreadStr+'</div></div></div>'+
...
function patchCard(id,live){
  ...
  // Real broker values
  const lotEl=document.getElementById('lot-'+id);
  if(lotEl&&live.minLot!=null)lotEl.textContent=Number(live.minLot).toFixed(3);
  const marginEl=document.getElementById('margin-'+id);
  if(marginEl&&live.minMargin!=null)marginEl.textContent='$'+Number(live.minMargin).toFixed(2);
  const spreadEl=document.getElementById('spread-'+id);
  if(spreadEl&&live.spreadDollar!=null)spreadEl.textContent='$'+Number(live.spreadDollar).toFixed(4);
  // Crossover chips
...
function updateHeaderStats(){
  let s=0;Object.values(liveData).forEach(l=>{s+=countActiveTFs(l);});
